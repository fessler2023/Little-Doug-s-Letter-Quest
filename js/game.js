...
async function create() {
    ...
    // Title
    this.add.text(LEFT_PANEL_WIDTH + RIGHT_PANEL_WIDTH / 2, 10, "Little Doug’s\nLetter Quest", { font: "32px Courier", fill: "#ffff00", stroke: "#ff00ff", strokeThickness: 2, align: 'center' });
    // Score & Level
...
function checkWordsOptimized(scene) {
    ...
    let foundWord = false;
    // Check all possible combinations of letters in the grid
    for (let startRow = 0; startRow < ROWS; startRow++) {
        for (let startCol = 0; startCol < COLS; startCol++) {
            if (!grid[startRow][startCol]) continue; // Skip empty cells
            let wordLength = 1;
            while (startRow + wordLength - 1 < ROWS && startCol + wordLength - 1 < COLS) {
                const word = grid.slice(startRow, startRow + wordLength).join("");
                if (dictionarySet.has(word)) {
                    ...
                    for (let row = startRow; row < startRow + wordLength; row++) {
                        for (let col = startCol; col < startCol + wordLength; col++) {
                            const letterObj = letters.find(l => l.x === col * CELL_SIZE && l.y === row * CELL_SIZE);
                            flashLetter(letterObj);
                        }
                    }
                }
            }
